...
const saveBlockAssets = (currentBlock: BlockEntity) => {
    ...
    return new Promise((resolve, reject) => {
      fetch(item)
        .then((res: any) => {
          if (res.status !== 200) {
            logseq.UI.showMsg(
              `链接: ${item} 请求失败，请检查链接或者去掉链接参数（问号及后面的部分）试试`,
              "error"
            );
            return reject(res);
          }
        ...
        .then((res) => {
          storage
            .setItem(
              decodeURIComponent(
                `${options[index].name}.${options[index].type}`
              ),
              res as any
            )
            .then((one) => {
  ...
  Promise.all(
    options.map((item, index) => {
       ...
       */
      const url = item.url?.includes("moffice-note")
        ? item.url
        : item.originalUrl;
    ...
    .then((paths) => {
      ...
      options.forEach((item, index) => {
        ...
        currentContent = item.mdImage
          ...
          : currentContent?.replace(
              item.originalUrl as string,
              `![${options[index].name}](${localPaths[index]})`
            );
      });
